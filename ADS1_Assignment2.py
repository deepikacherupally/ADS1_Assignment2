...
# pandas  will be used for data reading and initial analysis and also for some kind of visualization from dataframe
...
def corprep_df(ppel):    # function data preparation for indicator correlation
    '''Prepares data for indicator correlation '''
    ppelcols=ppel.columns    # columns of data (which will be converted to numeric)
...
def read_data(popelt): # reading data
    ...
    popel=popel[(popel['Indicator Name']==indwb[0])|((popel['Indicator Name']==indwb[1]))]    # data preaparation by indicators
    ...
    for i in range(len(indx)):
        nw.append(cntr[i])    # creating new column by combining country and indicator
